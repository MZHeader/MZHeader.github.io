...
A directory is created, containing the same binary (copied) and obfuscated settings.ini file.
...
A scheduled task is created, which points to the execution of this renamed Trickbot binary in the above directory.
...
Following execution flow with Procmon, we can see a svchost.exe process with the command line: "C:\Windows\system32\svchost.exe -k netsvcs -p -s Schedule", which indicates this is the schedueld task running.

This process spawns Vticmbqt.exe from the 'C:\Users\husky\AppData\Roaming\cashcore' directory, per the scheduled task.
...
We'll interrogate this process further with Process Hacker.

Find the svchost process > Properties > Memory > Strings

Filtering for any string containing "http" will help identify some C2 traffic.

Doing this shows interesting results for connections to IPv4 addresses over port 449, which Trickbot is known for using, so we'll change our filter to "449".
