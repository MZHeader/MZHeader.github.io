...
![CyberChef recipe URL decoding and gunzipping the payload byte array from the PowerShell script](/assets/img/e1c81006-928f-4863-956b-1ff2a84187a2.png)
...
![DetectItEasy identifying the extracted binary as a .NET executable](/assets/img/31baf012-8484-40ec-8768-8f2f10f2ece8.png)
...
![DNSpy showing heavily obfuscated Quasar RAT binary with renamed symbols](/assets/img/2600994a-708c-44b8-899e-19d766ee6983.png)
...
![De4Dot detecting and cleaning the obfuscated Quasar RAT binary](/assets/img/40b9706a-1afc-4417-bcfe-c2089f0bfd04.png)
...
![DNSpy showing de-obfuscated Quasar RAT binary with readable class structure](/assets/img/50dbb051-72e2-4c79-950b-356d578f07e8.png)
...
![DNSpy showing entry point calling configuration initialisation method shortly after startup](/assets/img/5fff0c0c-0ee9-4298-9089-3e544ff872ef.png)
...
![DNSpy showing GClass0 fields containing base64 encoded encrypted configuration strings](/assets/img/d9b4ff72-d9ea-482a-bea3-507cf4720124.png)

As well as the method which is used to decrypt and obtain the data.

![DNSpy showing smethod_0 decryption method called on all configuration strings](/assets/img/ed76a929-6459-44db-a94f-e0ca0703f45d.png)
...
![DNSpy showing Rfc2898DeriveBytes key derivation using the config password value](/assets/img/852a3607-8303-42c9-a7dd-a6ec2af05546.png)
...
![DNSpy showing PBKDF2 parameters: password, salt, 50000 iterations, SHA1](/assets/img/5fe61ec3-64a2-4c6e-ab56-1d2a5e1d85fb.png)
...
![DNSpy showing GClass30.byte2 field containing the PBKDF2 salt value](/assets/img/22eb8275-ac85-4e0a-8a1a-d146acacc313.png)

The result of this is the key to be used for AES decryption, evident from the use of `GClass.byte0`:

![DNSpy showing derived key bytes stored in GClass.byte0 for AES decryption](/assets/img/b86c5b66-d8ba-452a-81e6-84a0ced14659.png)

![DNSpy showing AES decryption routine using the PBKDF2-derived key](/assets/img/88ccb100-4570-4eb7-9eab-6459d53aa486.png)

We can create a SHA1 sum using these values by using a Derive PBKDF2 Key operator in CyberChef

![CyberChef PBKDF2 key derivation producing the AES decryption key](/assets/img/a5728e68-aaac-4db2-8ade-f865a46c1df9.png)
...
![CyberChef highlighting first 48 bytes of decoded config blob to identify hash and IV](/assets/img/6519cdb8-6b92-4086-8650-2d1859aef658.png)

The remaining bytes after the first 48 are our input which we are trying to decrypt.

![CyberChef AES-CBC decryption of Quasar RAT config string using derived key](/assets/img/bf2fd424-c995-468b-b1be-0de76e4abb34.png)

![CyberChef output revealing decrypted plaintext Quasar RAT configuration value](/assets/img/fd07aebb-f26e-4c55-be11-0ed23c0d9be9.png)
...
![DNSpy breakpoint set at the config decryption method to reveal plaintext values](/assets/img/b1d40550-d478-4dc8-b9b7-6f62624a322c.png)
...
![DNSpy debugger showing decrypted string value confirming Quasar RAT identity](/assets/img/3223cda0-3619-4bc6-8da6-0d43f1d66b6b.png)

**All Decoded Strings:**

![DNSpy locals window showing all decrypted Quasar RAT configuration strings including C2](/assets/img/9e0a669d-5817-48bc-b1c5-f89a0b08be49.png)
