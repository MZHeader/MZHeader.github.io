---
tags: CTF
description: A collection of write-ups for the Huntress 2024 CTF.
---
...
![Huntress CTF 2024 banner](https://github.com/user-attachments/assets/be7b4457-7099-454c-9ded-1ba0f8b468b4)
...
![Whamazon web terminal showing shop interface](https://github.com/user-attachments/assets/237a1e42-fd04-4c1d-aedd-f9517c529c07)

![Whamazon terminal showing negative quantity purchase granting infinite money](https://github.com/user-attachments/assets/4cc0bac1-ee6b-48fe-aee8-b87bd2aed61f)

![Whamazon terminal showing rock-paper-scissors game always choosing paper](https://github.com/user-attachments/assets/62385aff-fce4-4c7d-9052-6710c4330154)

![Whamazon inventory showing flag after purchasing it with infinite money](https://github.com/user-attachments/assets/20f3f53f-1b61-4d79-beca-c4bbded57494)
...
![GoCrackMe1 binary printing Access Denied on execution](https://github.com/user-attachments/assets/202077cd-6f8c-4fda-8679-f947f47d7f9e)
...
![disassembler showing conditional jump before Access Denied and flag branches in GoCrackMe1](https://github.com/user-attachments/assets/c576ea0f-6f27-4972-b98e-ce92eb447e50)
...
![GoCrackMe1 binary printing the flag after reversing the conditional jump](https://github.com/user-attachments/assets/47419767-dd0b-4f96-acd1-548242cc9366)
...
![EDB debugger showing Access Denied string in register values at 0x4f7a76](https://github.com/user-attachments/assets/f30442e7-8a57-4f61-92cc-9d72144f8273)
...
![EDB debugger stopped at 0x4f7a76 conditional jump with Zero Flag set to 1](https://github.com/user-attachments/assets/2cb52fad-0692-49a9-9f52-078346402b7c)

In EDB, change the Zero Flag by double clicking the Z under the registers view

![EDB registers panel showing Zero Flag being changed from 1 to 0](https://github.com/user-attachments/assets/ab9c4c3a-4ea5-44cc-9f50-49d49633245b)

Keep stepping through, at 0x4f7dc9 we're going to hit an instruction that jumps us to 0x4f81ab

![EDB disassembly at 0x4f7dc9 showing jump to flag writing loop at 0x4f81ab](https://github.com/user-attachments/assets/ce0540a5-5455-4766-bf1d-08053a3c157b)

This initiates a loop between 0x4f819e and 0x4f81ae

![EDB disassembly showing flag byte writing loop between 0x4f819e and 0x4f81ae](https://github.com/user-attachments/assets/909771ba-a0d1-401d-96db-6ae1383e8bf2)

We'll toggle a breakpoint at the following instruction 0x4f81b0 which we'll hit when the loop completes 

![EDB breakpoint set at 0x4f81b0 after the flag writing loop](https://github.com/user-attachments/assets/a1d3a9b8-de10-4231-bfd8-fedbb9b0f422)

Reviewing the RAX register we can see a string which could indicate part of the flag is being written

![EDB registers view showing partial flag string in RAX after first loop iteration](https://github.com/user-attachments/assets/345ba46b-1967-4dd3-940d-c5b4e3fb8fe2)
...
![EDB R12 register showing partial flag before program exits with Access Still Denied](https://github.com/user-attachments/assets/e84f1c5f-b2fa-49ef-bfc6-9605865d754d)
...
![EDB breakpoint at 0x4f7d82 with Zero Flag being set to 0 to continue the loop](https://github.com/user-attachments/assets/06aa1f16-c0b6-4c2f-a4e8-c5827cd56abb)

After changing our ZF and returning the loop, the entire flag will be written and visible in the R12 register

![EDB R12 register showing flag{ prefix being assembled in R12 after loop continues](https://github.com/user-attachments/assets/65ea1241-4d65-41ca-be1c-77b01251719f)
...
![EDB R12 register showing flag bytes being accumulated in correct order](https://github.com/user-attachments/assets/4dd5db46-9ee5-470c-91c7-474fa9e3053e)

After completing the loops a few times, we'll see the full flag

![EDB R12 register showing complete GoCrackMe3 flag after all loops complete](https://github.com/user-attachments/assets/69f5a9e0-42ad-4c57-b346-c271717fb440)
...
![decoded PowerShell command from the Russian Roulette shortcut base64 argument](https://github.com/user-attachments/assets/40778f66-372b-4877-b9f7-3738f5cbf3f8)
...
![batch script with garbled UTF-16LE encoded characters visible in text editor](https://github.com/user-attachments/assets/3aa48248-8763-4a2e-9469-6e445c54b97c)
...
![CyberChef UTF-16LE decode and re-encode fixing the batch script encoding](https://github.com/user-attachments/assets/516b15fb-d1d7-46c6-a0fa-6335a854f715)
...
![batch script after removing comment lines and cleaning up variable names](https://github.com/user-attachments/assets/1764cdde-a997-4879-b9f3-3544e7818c2c)

![batch script after further cleanup showing readable variable assignments](https://github.com/user-attachments/assets/b91782f8-0e75-43f7-b010-716bbddfa33c)

![cleaned batch script showing deobfuscated variable concatenation structure](https://github.com/user-attachments/assets/f184f5df-1681-44a6-b3ed-23ad27f56ede)

Execute the batch file, saving the output to a new file

![output of executed batch file showing resolved PowerShell command](https://github.com/user-attachments/assets/1cca6a64-2bf0-478c-bd21-778a6d0a027f)

Identify suspicious PowerShell command and decode the Base64 encoded string to reveal a new URL

![decoded base64 from PowerShell command revealing second stage URL](https://github.com/user-attachments/assets/84e79727-46f8-4ccc-82e7-2236f88182bf)
...
![CyberChef AES decryption of the flag string from the Russian Roulette PowerShell payload](https://github.com/user-attachments/assets/a2ee00ae-b8fc-48c8-9e0e-4ec24509f2ed)
...
![pyrchdata.com website showing Joe Davern listed as the security officer](https://github.com/user-attachments/assets/479e3b8b-8105-439c-89b9-77b87454730f)
...
![SMTP session sending phishing email as jdaveren@pyrchdata.com to swilliams](https://github.com/user-attachments/assets/9f80bc80-4990-4ff9-8d58-a2cc223c5da5)
