---
tags: Loader
description: Winos 4.0 is a multi-stage Windows loader delivered via trojanized installers. Investigation revealed its in-memory execution layers, process checks, and adaptive behavior depending on installed apps like WhatsApp and Telegram.
---
...
![WinDbg breakpoint on CreateFileA showing insttect.exe opening Single.ini](https://github.com/user-attachments/assets/82a5315e-653b-4291-b306-5e59c2db6e5d)

![WinDbg memory view showing file path Single.ini being passed to CreateFileA](https://github.com/user-attachments/assets/585ee1e7-8cd0-44ab-ba89-2cd090166349)

Following along until the next `VirtualAlloc` call, we can see in the ESI / EDI registers that there is a PE file in memory:

![WinDbg registers ESI and EDI showing PE header bytes in memory after VirtualAlloc](https://github.com/user-attachments/assets/6dae1fd8-f9f4-4c5c-b944-0207e79a590e)

![WinDbg memory view showing MZ and PE header of DLL loaded from Single.ini](https://github.com/user-attachments/assets/62629884-609f-4edf-83e0-70ba48635faa)

![WinDbg memory view showing further PE header bytes confirming executable in memory](https://github.com/user-attachments/assets/9ec325d5-9c80-4701-bdec-c6d88acdf6f4)
...
![Binary Refinery carve-pe peek output showing PE carved from Single.ini](https://github.com/user-attachments/assets/a14519a6-99ee-4415-a072-3c0837c96ad5)
...
![WinDbg showing carved DLL being called with VFPower export](https://github.com/user-attachments/assets/541eeb7a-56b8-45ff-a543-97f556ec3e4e)

Shortly after we notice the following mutex creation:

![WinDbg showing CreateMutex call creating the zhuxianlu mutex](https://github.com/user-attachments/assets/cbc0b715-ab6c-410c-9606-a9b7b2c0df62)


A reference to the `CreateToolhelp32Snapshot` API.

![WinDbg showing CreateToolhelp32Snapshot API call for process enumeration](https://github.com/user-attachments/assets/44172ca6-9c1f-46ec-a1c3-90da6cee6423)

And there is a reference to `360Tray.exe`.

![WinDbg memory view showing 360Tray.exe string being checked during process enumeration](https://github.com/user-attachments/assets/95574c05-a5ef-4f78-bf67-fb81bb21daa1)
...
![IDA Pro showing global pNodeName variable resolving to C2 IP 120.89.71.130](https://github.com/user-attachments/assets/81861b1f-9573-4ea2-ab72-0fcf3309d0b4)
...
![IDA Pro decompiler showing mutex creation with value zhuxianlu in the second loader DLL](https://github.com/user-attachments/assets/b35d128a-886b-446b-9796-a58d3e27addc)
...
![IDA Pro showing two base64 blobs in sub_1000AB80 used to create a scheduled task](https://github.com/user-attachments/assets/44d137ad-9e50-4343-870f-9657103c14dd)
...
![IDA Pro showing Config2.ini loader DLL C2 address 43.226.125.17 used for Telegram/WhatsApp path](https://github.com/user-attachments/assets/bcb8707c-73e3-48c1-97da-4c8d78e89f8f)
