...
![maldoc enable content lure prompt](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/da75e6b8-26e0-4d18-ba83-fd86a19bcdd9)

We can interrogate this macro by using OLE tools to view it.

![OLE tools listing streams in the maldoc](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/c521ee37-e76b-4b24-b3f7-7bf4dd102d94)

![OLE tools showing macro code with VirtualAlloc and CreateThread calls](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/66db0700-4e12-4eba-94e4-6621bb1b38d7)
...
![word document Alternative Text field containing hidden base64 payload](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/4c944195-0f30-4bdb-bf49-6967865241bd)

I found the full string by querying the `Data` stream, using OLE tools.

![OLE tools Data stream showing base64 encoded shellcode blob](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/fe007626-91e1-4cc1-b51f-5667a6ddfdf0)

_Base64 Snippet:_

![base64 encoded Donut shellcode snippet extracted from OLE stream](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/53b37b49-c1de-4051-83c8-fae9faddce0c)
...
![Blobrunner loading shellcode and x32dbg attaching to the process](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/0734d4a3-15b5-4077-8998-04858c32b2cf)

We'll set a breakpoint in x32dbg for the base address `0x012d0000` and run the shellcode.

![x32dbg breakpoint hit at shellcode base address 0x012d0000](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/7bc92678-f8d7-4d58-94ec-47861d5d2bbd)
...
![API Monitor showing netflex.exe file path reference in AppData](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/fcb9df18-624f-4385-9615-51c036dd2b14)

We also see indications that a registry run key is going to be a form of persistence for this malware.

![API Monitor showing registry run key write for persistence](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/8be53c65-f5cc-41aa-824c-8963e1810e88)
...
![API Monitor NtWriteFile call showing first bytes of a PE being written to disk](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/db05a2ba-1cb1-4cbc-821e-be7dbfa3e387)

We'll set a breakpoint in x32dbg with `bp NtWriteFile` and run until that breakpoint is met.

![x32dbg breakpoint on NtWriteFile with stack showing MZ header address](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/703b708b-975a-476c-8d35-e203f022ddcf)
...
![x32dbg dump view showing MZ DOS header of embedded PE](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/7f3a3a6e-c861-4025-a930-276a3beb477e)

![x32dbg dump view showing PE header bytes confirming executable format](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/6738a72d-f15c-4cf1-81ef-0d653d8be98f)
...
![x32dbg memory map showing region containing the PE to be dumped](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/0433f1a5-718c-47c4-8673-531b4bc67e02)

![x32dbg dump memory dialog saving the extracted PE to disk](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/f39d3fb0-becb-4b17-adbc-35bc1ed4856c)
...
![HxD hex editor showing raw memory dump with data before MZ header to be removed](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/3595c7dc-21f8-4909-9b56-f90d86b74120)

## 1st Executable - The Loader

This is a .NET binary so we will run it through DNSpy to figure out what it's doing.

![DNSpy showing .NET loader entry point and class structure](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/28ef07f4-5a8a-421d-8b18-d8acbca8338c)

This appears to be a loader with the injection target of `svchost.exe`.

![DNSpy decompiled code showing process injection targeting svchost.exe](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/63e29b47-8ecd-4118-851d-2af3fdde3d59)

![DNSpy showing injection API calls VirtualAllocEx and WriteProcessMemory](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/d7f5cee7-f6c5-4a30-9ee1-6772dec017dc)

We see more references to the registry run key previously mentioned.

![DNSpy showing registry run key persistence code in the loader](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/fe9edb4b-affa-4298-8438-e382afd5baf7)

And, what we're interested in - a baes64 encoded chunk and target file path.

![DNSpy showing base64 encoded payload blob and target drop path in loader](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/1cdab8a4-e617-4b50-b587-f7fb14bf0fb3)

A From Base64 operation will reveal our next binary, dropped from this loader.

![CyberChef From Base64 operation revealing MZ header of dropped executable](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/a75e9850-0fe6-4b3b-8ab9-ee938484a6d4)
...
![DNSpy showing Win32_CacheMemory WMI query used for VM detection](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/6882b2c3-d2b7-4700-b20d-9296049a7200)
...
![DNSpy showing AMSI and ETW bypass routine in netflex.exe](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/48e550fc-4b5e-4fcf-9e93-438232dbdc9d)


Next up is the main function, which essentially decrypts and executes a payload in memory.

![DNSpy showing main decryption function with AES key derivation and base64 decode](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/af3e374a-7c42-4e6f-8775-aae8ad36c471)
...
![DNSpy showing GZip decompress function that strips the first 4 bytes before decompression](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/a0e449f9-0ef4-4d75-b12f-19acbe97c7dd)
...
![DNSpy Settings class showing baseData, aes_key, and aes_iv fields](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/1f4f0997-0af7-488a-bfde-bdd9bfda3e2d)

![DNSpy Settings class showing encrypted baseData blob value](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/6f646fb8-e4a7-4d4b-b7db-b210541829ea)
...
![CyberChef recipe output revealing decrypted .NET executable from netflex.exe](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/7e120cd3-88a3-4b70-a9ee-f61e6bba9413)
...
![DNSpy showing NJRat C2 domain netflex.duckdns.org and installation directory](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/6f7fd25f-61d4-4658-bfac-a22c4f894da3)
...
![DNSpy showing removable drive replication routine in NJRat](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/381c8cc6-fca3-42da-ad73-5d1b4f3e790f)

![DNSpy showing VBS script persistence creation code](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/8eb4112f-fa44-46a2-924a-711cf073aca0)

![DNSpy showing VBS script content written for persistence](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/b47f825d-f9e8-4a07-b3be-14859f79be54)
...
![DNSpy showing HKCU Run key write for NJRat persistence](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/d6b7e8d2-0be3-448d-9eb2-f1bb0ec4b51c)

![DNSpy showing registry key path being constructed for run key persistence](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/d7224c19-bd8c-4827-a730-1183d14bfcc3)

![DNSpy showing registry SetValue call writing NJRat to HKCU Run key](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/f4bf9291-5090-486c-bffd-33e51d3adcc3)

As well as the binary being copied to the Startup directory.

![DNSpy showing Startup directory copy for additional persistence](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/27e7f7c9-0c18-4648-9119-7acc0357a85d)

**Keylogging**

The `kl` class in the binary presents the keylogging functionality.

![DNSpy showing kl keylogger class with keystroke capture methods](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/baf8b432-0770-42ea-a296-6c91518e97c6)

![DNSpy showing keylogger hook installation code in NJRat](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/4795b74c-8d51-4210-94a7-74621f3628e0)

The following line defines where the keystrokes are to be recorded.

![DNSpy showing keylog storage path set to HKCU\SOFTWARE\Netflex registry key](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/e42dd246-0f58-4113-bcfb-f22eb70d79db)

**STV Function**

![DNSpy showing STV function that writes captured keystrokes to the registry](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/0bee5133-49f0-4b5a-9674-7d18cde86dc5)


With this, we know that keystrokes should be recorded under the `HKCU\SOFTWARE\Netflex` registry key.

![DNSpy showing HKCU\SOFTWARE\Netflex registry key used as keylog storage](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/9c582bd7-186c-400d-983d-ea2deddfd01e)

**Basic Execution Flow**


![DNSpy showing high-level execution flow of NJRat payload](https://github.com/MZHeader/MZHeader.github.io/assets/151963631/fba213b3-133c-453b-aa70-e2f92e48b105)
